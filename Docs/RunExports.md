...
`context.frameTimingVersion = 2` fixes FPS conversion from seconds (`1 / dt`).
Earlier builds accidentally exported FPS and minimum-FPS values at 1000x;
their `frameMs`, histograms and slow-frame counts remain valid. Prefer those
frame-duration fields when analyzing older exports; do not interpret their
minimum-FPS fields literally.

Version 2 also adds `samples[].cpu`, a bounded window of completed combat
Update calls since the previous sample. `frames` is the window denominator;
`simulationMeanMs`, `renderMeanMs`, `hudMeanMs`, `updateMeanMs`, and `updateMaxMs`
measure main-thread script elapsed time. Simulation includes journey stepping;
render covers script-side view updates, not GPU drawing; HUD covers UpdateHud,
while total Update also includes UI reconciliation and exporter work. The
current unfinished Update is included in the next window. `gen0Collections`
counts GC collections since the previous sample, and `managedBytes` samples
the managed heap without forcing collection. Empty windows have zero timings.
These are not full CPU/GPU profiler measurements, allocation-byte counts, or
the cause of a stall by themselves. Unity canvas rebuilds, engine rendering,
GPU waits and OS scheduling can occur outside the measured script work.
The fields travel through the existing one-second sample JSON/JSONL path;
there are no per-frame history events or extra file writes.
...
- `arena_map_policy`: Null City geometry, viewport and purge policy once/visit;
  use its versioned ID to distinguish original4× integration from native1× correction.
