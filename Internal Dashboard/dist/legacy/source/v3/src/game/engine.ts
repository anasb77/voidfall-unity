...
export class Game {
  ...
  private chB(){
    ...
    if(this.formId==="overseer")b.droneN=1;if(this.formId==="overseer")b.droneDmg=1.25;return b;
  }
  ...
  private reset(){
    ...
    this.lH={hp:-1,mhp:-1,xp:-1,lv:-1,t:-1,sc:-1,k:-1,cb:-1,cr:-1,sh:-1,mg:-1};
  ...
  private pkType():EnemyId{const t=this.time;const r=Math.random();if(t<12)return"chaser";if(t<30)return r<0.6?"chaser":"runner";if(t<55)return r<0.4?"chaser":r<0.6?"runner":r<0.75?"spider":"shooter";if(t<80)return r<0.3?"chaser":r<0.45?"runner":r<0.55?"spider":r<0.7?"shooter":r<0.8?"wisp":"charger";if(t<120)return r<0.2?"chaser":r<0.35?"runner":r<0.45?"spider":r<0.55?"shooter":r<0.65?"wisp":r<0.75?"charger":r<0.85?"exploder":"swarmer";return r<0.15?"chaser":r<0.28?"runner":r<0.38?"spider":r<0.48?"shooter":r<0.56?"wisp":r<0.65?"charger":r<0.75?"exploder":r<0.82?"phantom":r<0.88?"swarmer":r<0.94?"shielded":r<0.97?"golem":"splitter";}
  ...
  private fireW(dt:number){
    for(const w of WEAPONS){const lv=this.wlv[w.id]||0;if(lv<=0)continue;const evo=this.wevo[w.id]||false;const ws=weaponStats(w.id,lv,evo);
      ...
      if((this._cd[w.id]||0)<=0){this._cd[w.id]=ws.cooldown*this.stat("fr");this.fireOne(w.id,dmg,ws,evo,dt);}}}
  ...
  private upEns(dt:number){
    ...
    for(let i=this.ens.length-1;i>=0;i--){const e=this.ens[i];e.age+=dt;if(e.hit>0)e.hit-=dt;if(e.atkCd>0)e.atkCd-=dt;if(e.bladeCd>0)e.bladeCd-=dt;
      ...
      switch(e.type){
        ...
        case"phantom":e.stateT-=dt;if(e.stateT<=0||dist>400){e.stateT=2+Math.random()*3;if(dist<500){e.x=this.px+(Math.random()-0.5)*200;e.y=this.py+(Math.random()-0.5)*200;e.age=0.3;this.burst(e.x,e.y,"indigo",6,80,0.3,0.5);}}e.vx=nx*spd;e.vy=ny*spd;break;
  ...
  private render(){
    ...
    if(this.palive){const f=this.form;const blink=this.pifr>0&&Math.sin(this.aT*34)>0;ctx.globalAlpha=blink?0.35:1;ctx.save();ctx.translate(this.px,this.py);ctx.rotate(this.aT*1.4);
      ...
      ctx.drawImage(this.sp.playerRing,-31,-31,62,62);ctx.restore();
      // form-colored player
      const ps=this.sp.player.width;ctx.drawImage(this.sp.player,this.px-ps/2,this.py-ps/2);
