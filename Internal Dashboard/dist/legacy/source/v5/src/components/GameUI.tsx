...
import { Play, Pause, RotateCcw, Home, Volume2, VolumeX, Trophy, Skull, Heart, Crown, ChevronRight, Zap, Shield, type LucideIcon } from "lucide-react";
...
function ScoreTable({ scores, highlight }: { scores: HighScore[]; highlight?: number }) {
  if (!scores.length) return <div className="py-6 text-center text-[13px] tracking-widest text-slate-500 font-display">NO RUNS RECORDED</div>;
  return (
...
export default function GameUI() {
  ...
  const form = FORMS.find(f => f.id === selForm) || FORMS[0];
  const startRun = () => { eng().setRun(selMode, selArena as any, selForm as any, null); eng().start(); };
  const _upgList = Object.entries(wLv).filter(([, n]) => n > 0); void _upgList;
          ...
          <div className="flex items-start gap-2">
            <div className="text-right">
              ...
              <div className="mt-1 flex items-center justify-end gap-1">
                <span className="text-xs">{form?.magic.icon}</span>
                <span className="font-display text-[9px] text-slate-400" style={{color:form?.magic.color}}>{form?.magic.name}</span>
              </div>
            </div>
            <IconBtn icon={Pause} label="Pause" onClick={() => eng().togglePause()} />
          </div>
        </div>
        {wLv && Object.entries(wLv).filter(([,n])=>n>0).length > 0 && (
          <div className="absolute bottom-3 left-3 flex flex-wrap gap-1.5 sm:bottom-4 sm:left-5">
            {Object.entries(wLv).filter(([,n])=>n>0).map(([id, n]) => {
              const def = WEAPON_MAP[id]; if (!def) return null;
              const isActive = (wLv as any)._active?.includes(id);
              return (
                <div key={id} title={`${def.name} Lv${n}${isActive?' (ACTIVE)':''}`}
                  className={`relative flex h-9 w-9 items-center justify-center rounded-md border backdrop-blur-sm ${isActive ? 'border-cyan-300/50 bg-cyan-400/15 text-cyan-200' : 'border-slate-600/30 bg-slate-950/40 text-slate-500'}`}>
                  <span className="text-xs">{def.icon}</span>
                  <span className="absolute -bottom-1 -right-1 rounded-sm px-1 font-display text-[9px] font-bold leading-3 text-slate-950" style={{backgroundColor:isActive?'#22d3ee':'#64748b'}}>{n}</span>
                  {isActive && <span className="absolute -top-1 -left-1 h-2 w-2 rounded-full bg-cyan-400 animate-pulse"></span>}
                </div>
              );
            })}
          </div>
        ...
        <div className="anim-fade-in absolute inset-0 flex items-center justify-center overflow-y-auto px-4 py-6">
          <div className="pointer-events-auto flex w-full max-w-xl flex-col items-center">
            <div className="anim-title-float flex flex-col items-center mb-6">
              ...
              <div className="mt-2 font-display text-[11px] tracking-[0.5em] text-slate-400 sm:text-sm">SURVIVOR OF THE SWARM v1.1</div>
            </div>

            <div className="flex gap-2 mb-4 flex-wrap justify-center">
              <button onClick={() => { setShowForm(!showForm); setShowArena(false); }}
                className={`btn-neon px-4 py-2 rounded-md text-xs font-display tracking-wider ${showForm ? "bg-cyan-400/30" : ""}`}>
                ✧ FORM
              </button>
              <button onClick={() => { setShowArena(!showArena); setShowForm(false); }}
            ...
            {/* Form info */}
            <div className="mb-3 text-center panel rounded-lg px-4 py-2">
              <div className="flex items-center gap-3 justify-center">
                <span className="font-display text-sm font-bold tracking-wider" style={{ color: form.color }}>
                  {form.name}
                </span>
                <span className="text-slate-600">|</span>
                <span className="font-display text-xs text-slate-400">
                  {WEAPON_MAP[form.startWeapon]?.icon} {WEAPON_MAP[form.startWeapon]?.name}
                </span>
                <span className="text-slate-600">|</span>
                <span className="font-display text-xs" style={{ color: form.magic.color }}>
                  {form.magic.icon} {form.magic.name} ({form.magic.cooldown}s cd)
                </span>
              </div>
              <div className="flex flex-wrap gap-2 justify-center mt-1 text-[10px] text-slate-500">
                <span>❤ {form.hp > 1 ? '+' : ''}{Math.round((form.hp - 1) * 100)}% HP</span>
                <span>⚡ {form.speed > 1 ? '+' : ''}{Math.round((form.speed - 1) * 100)}% SPD</span>
                <span>⚔ {form.dmg > 1 ? '+' : ''}{Math.round((form.dmg - 1) * 100)}% DMG</span>
                <span>✦ {(form.crit * 100).toFixed(0)}% CRIT</span>
                {form.armor > 0 && <span>🛡 {form.armor} ARM</span>}
                {form.regen > 0 && <span>♥ {Math.round(form.regen * 1000) / 10}% REGEN</span>}
                <span className="text-cyan-300">◈ 2 WEAPON SLOTS</span>
              </div>
            </div>
            ...
            {showForm && (
              <div className="anim-rise-in w-full grid grid-cols-2 gap-2 mb-4">
                {FORMS.map((f) => {
                  const sw = WEAPON_MAP[f.startWeapon];
                  return (
                    <button key={f.id} onClick={() => { setSelForm(f.id); setShowForm(false); }}
                      className={`panel rounded-lg p-3 text-left transition border-l-4 ${selForm === f.id ? "ring-2 ring-cyan-300/50 bg-cyan-400/5" : ""} hover:bg-slate-800/50`}
                      style={{ borderLeftColor: f.color }}>
                      <div className="flex items-center gap-2">
                        <div className="w-3 h-3 rounded-full" style={{ backgroundColor: f.color, boxShadow: `0 0 8px ${f.color}66` }}></div>
                        <div className="font-display text-sm font-bold" style={{ color: f.color }}>{f.name}</div>
                      </div>
                      ...
                      <div className="mt-1 flex flex-wrap gap-1.5 text-[10px] text-slate-500">
                        <span className="px-1.5 py-px rounded bg-slate-800/80">❤{f.hp > 1 ? '+' : ''}{Math.round((f.hp-1)*100)}%</span>
                        <span className="px-1.5 py-px rounded bg-slate-800/80">⚡{f.speed > 1 ? '+' : ''}{Math.round((f.speed-1)*100)}%</span>
                        <span className="px-1.5 py-px rounded bg-slate-800/80">⚔{f.dmg > 1 ? '+' : ''}{Math.round((f.dmg-1)*100)}%</span>
                        <span className="px-1.5 py-px rounded bg-slate-800/80">✦{(f.crit*100).toFixed(0)}%</span>
                        {f.armor>0 && <span className="px-1.5 py-px rounded bg-slate-800/80">🛡{f.armor}</span>}
                        {sw && <span className="px-1.5 py-px rounded bg-slate-800/80">{sw.icon} {sw.name}</span>}
                      </div>
                      <div className="mt-1 text-[10px] flex items-center gap-1" style={{ color: f.magic.color }}>
                        <span>{f.magic.icon}</span>
                        <span className="font-semibold">{f.magic.name}</span>
                        <span className="text-slate-500">({f.magic.cooldown}s)</span>
                      </div>
                      <div className="text-[10px] text-slate-600 mt-0.5">{f.magic.desc}</div>
                    </button>
                  );
                })}
              </div>
            ...
            {showArena && (
              <div className="anim-rise-in w-full grid grid-cols-2 gap-2 mb-4">
                {[
                  { id: "neon", name: "Neon District", desc: "Balanced. The void's front line.", color: "#22d3ee", mods: "Standard difficulty" },
                  { id: "ash", name: "Ash Wastes", desc: "Choking dust reduces visibility.", color: "#fb923c", mods: "+15% HP, +20% elites" },
                  { id: "reactor", name: "Reactor Core", desc: "Electrical hazards and unstable energy.", color: "#34d399", mods: "+25% HP, +50% elites" },
                ].map((a) => (
                  <button key={a.id} onClick={() => { setSelArena(a.id); setShowArena(false); }}
                    className={`panel rounded-lg p-3 text-left transition border-l-4 ${selArena === a.id ? "ring-2 ring-cyan-300/50 bg-cyan-400/5" : ""} hover:bg-slate-800/50`}
                    style={{ borderLeftColor: a.color }}>
                    <div className="flex items-center gap-2">
                      <div className="w-3 h-3 rounded-full" style={{ backgroundColor: a.color, boxShadow: `0 0 8px ${a.color}66` }}></div>
                    ...
                    <div className="mt-1 text-[10px] text-yellow-200/70">⚠ {a.mods}</div>
                  </button>
                ))}
              </div>
            )}
            ...
            <div className="mt-4 flex items-center gap-3">
              ...
              <div className="font-display text-[10px] tracking-[0.3em] text-slate-600">v1.1 — Q/E for Magic</div>
            </div>
          </div>
        </div>
      )}
          ...
          <div className="pointer-events-auto flex w-full max-w-4xl flex-col items-center">
            ...
            <div className="flex w-full flex-col justify-center gap-3 sm:flex-row sm:gap-4">
              {opts.map((id, i) => {
                ...
                const Icon = parsed.type === "heal" ? Heart : parsed.type === "reroll" ? RotateCcw : parsed.type === "banish" ? Shield : Zap;
                const rarityColor = wDef?.rarity === "legendary" ? "text-yellow-300" : wDef?.rarity === "epic" ? "text-fuchsia-300" : wDef?.rarity === "rare" ? "text-cyan-300" : "text-slate-300";
                return (
                  <button key={id + i} onClick={() => eng().applyUpgrade(id)}
                    ...
                    <span className="flex min-w-0 flex-col items-center">
                      <span className={`font-display text-[9px] tracking-[0.35em] ${rarityColor}`}>
                        {parsed.type === "weapon" ? (wDef?.rarity || "").toUpperCase() : parsed.type === "passive" ? (pDef?.rarity || "").toUpperCase() : parsed.type.toUpperCase()}
                      </span>
                      <span className="font-display text-lg font-bold tracking-wider text-slate-100">
                        {parsed.type === "heal" ? "HEAL" : parsed.type === "reroll" ? "REROLL" : parsed.type === "banish" ? "BANISH" : wDef?.name || pDef?.name || id}
                      </span>
                      <span className="mt-1 text-[12.5px] leading-snug text-slate-400">
                        {parsed.type === "heal" ? "Restore 40 HP" : parsed.type === "reroll" ? "Re-roll choices" : parsed.type === "banish" ? "Remove one choice" :
                          wDef ? `Lv ${wLv[parsed.key] || 1} → Lv ${(wLv[parsed.key] || 0) + 1}` :
                          pDef ? pDef.description((wLv[parsed.key] || 0) + 1) : ""}
                      </span>
                      {wDef?.evolution && (wLv[parsed.key] || 0) >= 7 && <span className="mt-1 text-[10px] text-yellow-300 font-display tracking-wider">NEXT: EVOLUTION</span>}
                    </span>
        ...
        <div className="anim-fade-in absolute inset-0 flex items-center justify-center overflow-y-auto bg-slate-950/55 px-4 py-6 backdrop-blur-[2px]">
          <div className="pointer-events-auto flex w-full max-w-md flex-col items-center">
            ...
            <div className="anim-rise-in mt-5 grid w-full grid-cols-2 gap-2.5" style={{ animationDelay: "80ms" }}>
              ...
              {[
                { label: "KILLS", value: String(over.stats.kills) },
