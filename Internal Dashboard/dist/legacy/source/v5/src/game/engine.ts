...
export class Game {
  ...
  activeW:WeaponId[]=[]; // 2 active weapon slots
  private readonly maxActiveW = 2;
  spT=0.9;elT=40;swT=25;bossT:number[]=[300,600,900];bIdx=0;tIdx=0;km=50;
  private eventT=75;private eventEnd=0;private eventKind="";
  ...
  stat(n:string){
    ...
    switch(n){
      case"maxHp":return Math.round(100*cb.hp*(1+pl("maxHp")*0.2)*(pl("glassCore")?0.65:1));
      case"baseHp":return cb.hp;
      case"dmg":{let d=cb.dmg*(1+pl("damage")*0.12)*(1+(pu.startDmg||0)*0.05);if(this.lastStandActive)d*=2;if(pl("glassCore"))d*=1.45;return d;}
      ...
      case"speed":return 235*cb.speed*Math.pow(1.08,pl("moveSpeed"))*(1+(pu.moveSpd||0)*0.03)*(pl("glassCore")?1.25:1);
      ...
      case"armorBreak":return 1+pl("armorBreak")*0.12;
      case"volatile":return pl("volatile")*1.5;
      case"weaponMastery":return 1+pl("weaponMastery")*0.08*Math.max(0,this.activeW.length);
      case"hollowPoint":return 1+pl("hollowPoint")*0.1;
      case"manaCore":return 1-pl("manaCore")*0.1;
      case"glassCore":return pl("glassCore")?1.45:1;
      default:return 1;
    }
  }
  private chB(){
  ...
  private reset(){
    ...
    this.spT=0.9;this.elT=this.arenaM.eliteF*40;this.swT=25;this.eventT=75;this.eventEnd=0;this.eventKind="";
  ...
  activateMagic(){
    ...
    this.magicCd=f.magic.cooldown*this.stat("manaCore");this.magicT=f.magic.duration;this.magicOn=true;this.magicId=f.id;
    this.sfx.magic();
    switch(f.id){
      ...
      case"phantom":{this.pifr=1.5;this.magicOn=true;this.magicT=1.5;this.toast("PHASE RUSH","info");this.burst(this.px,this.py,"fuchsia",14,200,0.4,0.7);this.shake(0.15);break;}
  ...
  private roll():string[]{
    ...
    for(const w of WEAPONS){const lv=this.wlv[w.id]||0;if(lv>0&&lv<8)avail.push({id:`w_${w.id}`,w:6});if(lv>=8&&!this.wevo[w.id])avail.push({id:`w_${w.id}`,w:3});if(lv===0&&this.activeW.length<this.maxActiveW)avail.push({id:`w_${w.id}`,w:2.5});}
  ...
  applyUpgrade(id:string){
    ...
    if(p.t==="weapon"){this.wlv[p.k as WeaponId]=(this.wlv[p.k as WeaponId]||0)+1;this.save.favWeapon[p.k as WeaponId]=(this.save.favWeapon[p.k as WeaponId]||0)+1;
      ...
      if(!this.activeW.includes(wk)){
        if(this.activeW.length<this.maxActiveW){this.activeW.push(wk);}
  ...
  private spDir(dt:number){
    ...
    // Rotating micro-events add authored pressure without changing the core loop.
    if(this.time>=this.eventT){
      this.eventT+=75;
      const events=["bloodmoon","salvage","riftstorm","overrun"];
      this.eventKind=events[Math.floor(Math.random()*events.length)];
      this.eventEnd=this.time+18;
      if(this.eventKind==="bloodmoon"){this.elT=Math.min(this.elT, this.time+8);this.toast("BLOOD MOON — ELITES HUNT","danger");}
      if(this.eventKind==="salvage"){for(let i=0;i<10;i++){const a=Math.random()*TAU;const rr=160+Math.random()*260;this.gems.push({x:this.px+Math.cos(a)*rr,y:this.py+Math.sin(a)*rr,vx:0,vy:0,val:12,tier:1,age:0,pull:false,spd:0,kind:"credit"});}this.toast("SALVAGE SHOWER","gold");}
      if(this.eventKind==="riftstorm"){for(let i=0;i<12;i++)this.spE("rift",this.px+(Math.random()-0.5)*700,this.py+(Math.random()-0.5)*500,0.65);this.toast("RIFT STORM","danger");}
      if(this.eventKind==="overrun"){for(let i=0;i<22;i++)this.spE(i%3===0?"jumper":"crawler");this.toast("OVERRUN","danger");}
    }
  ...
  private rollEM(){const all=["giant","frenzied","shielded","regen","explosive","vampiric","electrified"];const n=1+(this.time>200?1:0);const m:string[]=[];for(let i=0;i<n;i++)m.push(all[Math.floor(Math.random()*all.length)]);return m;}
  ...
  private shake(a:number){
    // Respect saved accessibility settings and never allow additive runaway trauma.
    const intensity=this.save.settings.reducedMotion?0.12:Math.max(0,Math.min(1,this.save.settings.shakeIntensity));
    this.trauma=Math.min(0.48,this.trauma+Math.min(0.12,a*intensity));
  }
  ...
  private update(dt:number,dr:number){
    ...
    this.trauma=Math.max(0,this.trauma-dr*3.8);this.rFl=Math.max(0,this.rFl-dr*2.4);this.cFl=Math.max(0,this.cFl-dr*2.2);this.gFl=Math.max(0,this.gFl-dr*2.0);
  ...
  private upEns(dt:number){
    ...
    for(let i=this.ens.length-1;i>=0;i--){const e=this.ens[i];e.age+=dt;if(e.hit>0)e.hit-=dt;if(e.atkCd>0)e.atkCd-=dt;if(e.bladeCd>0)e.bladeCd-=dt;
      ...
      switch(e.type){
        ...
        case"wraithlord":{e.stateT-=dt;if(e.stateT<=0){e.stateT=7;const pulse=220;if(Math.hypot(this.px-e.x,this.py-e.y)<pulse){const pd=Math.hypot(this.px-e.x,this.py-e.y)||1;this.dmgP(e.dmg*0.35,(this.px-e.x)/pd,(this.py-e.y)/pd);}this.toast("WRAITH LORD — GRAVE PULSE","danger");this.ring(e.x,e.y,16,pulse*2,0.45);this.burst(e.x,e.y,"wraithlord",10,100,0.5,0.6);}e.vx=nx*spd*0.8;e.vy=ny*spd*0.8;break;}
      ...
      // Contact damage is only valid while the enemy is inside the player-centered
      // playfield. This prevents off-screen/recycled enemies from hitting invisibly.
      const visibleRange=Math.max(this.w,this.h)*0.58+e.r;
      const inPlayfield=Math.abs(e.x-this.px)<this.w*0.58+e.r&&Math.abs(e.y-this.py)<this.h*0.58+e.r;
      if(this.palive&&e.age>0.55&&e.atkCd<=0&&this.pifr<=0&&!e.burrowed&&inPlayfield){if(dist<e.r+14&&dist<visibleRange){e.atkCd=0.7;this.dmgP(e.dmg,nx,ny);if(e.type==="exploder")this.killE(e,i);}}
  ...
  private chainLt(from:E,n:number,d:number){if(n<=0)return;let best:E|null=null,bd=180;for(const e of this.ens){if(e===from||e.burrowed)continue;const d2=Math.hypot(e.x-from.x,e.y-from.y);if(d2<bd){bd=d2;best=e;}}
    if(best){const crit=Math.random()<this.stat("crit");this.dmgE(best,d*(crit?this.stat("critDmg"):1),best.x-from.x,best.y-from.y,80,crit,false);this.chainLt(best,n-1,d*0.75);}}
  private exBul(b:B){for(const e of this.ens){if(e.burrowed)continue;const d=Math.hypot(e.x-b.x,e.y-b.y);if(d<(b.explodeR||40)+e.r)this.dmgE(e,b.dmg*0.5,b.x-e.x,b.y-e.y,120,false);}const volatile=this.stat("volatile");this.burst(b.x,b.y,"cyan",8,200,0.4,0.7);if(volatile>0)this.ring(b.x,b.y,12,(b.explodeR||40)*1.5,volatile*0.35);}

  private dmgE(e:E,d:number,dx:number,dy:number,knock:number,crit:boolean,allowChain=true){
    const mark=e.markT>0?this.markMult:1;d*=mark;
    if(e.elite)d*=this.stat("eliteDmg");
    if(e.shield>0||e.type==="tank"||e.type==="golem"||e.type==="colossus")d*=this.stat("armorBreak");
    // Executioner: bonus damage to low HP enemies
    if(e.hp/e.maxHp<0.25)d*=this.stat("executioner");
    if(e.hp/e.maxHp<0.5)d*=this.stat("hollowPoint");
    ...
    const cl=this.stat("chainLightning");if(allowChain&&cl>0)this.chainLt(e,cl,d*0.5);
  ...
  private render(){
    ...
    // Smooth deterministic shake avoids random per-frame camera jitter.
    const sm=Math.min(6,this.trauma*this.trauma*10);
    const shX=Math.sin(this.aT*73.1)*sm;
    const shY=Math.cos(this.aT*91.7)*sm;
    ...
    for(let i=0;i<this.ens.length;i++){const e=this.ens[i];const s=this.sp.enemy[e.type]||this.sp.enemy.chaser;const grow=e.age<0.35?Math.min(1,e.age/0.35):1;const sz=s.width*grow;
      // Telegraph high-danger attacks before the hit arrives.
      if(e.type==="charger"||e.type==="lancer"||e.type==="jumper"){
        const charging=e.state===1||e.state===2;ctx.globalAlpha=charging?0.45:0.12;ctx.strokeStyle=e.type==="jumper"?"#f97316":"#ef4444";ctx.lineWidth=charging?3:1;ctx.beginPath();ctx.moveTo(e.x,e.y);ctx.lineTo(e.x+(this.px-e.x)*0.7,e.y+(this.py-e.y)*0.7);ctx.stroke();ctx.globalAlpha=1;
      }
      if(e.type==="sniper"){ctx.globalAlpha=0.22+0.14*Math.sin(this.aT*8);ctx.strokeStyle="#ef4444";ctx.lineWidth=2;ctx.beginPath();ctx.moveTo(e.x,e.y);ctx.lineTo(this.px,this.py);ctx.stroke();ctx.globalAlpha=1;}
      if(e.type==="siren"||e.type==="pulsar"){ctx.globalAlpha=0.16+0.08*Math.sin(this.aT*5);ctx.strokeStyle=e.type==="siren"?"#f472b6":"#e879f9";ctx.lineWidth=2;ctx.beginPath();ctx.arc(e.x,e.y,e.r+10+Math.sin(this.aT*4)*4,0,TAU);ctx.stroke();ctx.globalAlpha=1;}
      ctx.save();ctx.translate(e.x,e.y);ctx.rotate(e.rot);ctx.globalAlpha=e.burrowed?0.18:e.summoned?0.7:1;ctx.drawImage(s,-sz/2,-sz/2,sz,sz);ctx.globalAlpha=1;ctx.restore();
      if(e.burrowed){ctx.globalAlpha=0.35+Math.sin(this.aT*8)*0.12;ctx.strokeStyle="#fbbf24";ctx.lineWidth=1;ctx.beginPath();ctx.arc(e.x,e.y,e.r+5,0,TAU);ctx.stroke();ctx.globalAlpha=1;}
    ...
    ctx.restore();
    // Keep dangerous off-screen enemies readable instead of letting them hit from nowhere.
    for(const e of this.ens){
      if(e.burrowed||!(e.type==="charger"||e.type==="lancer"||e.type==="jumper"||e.type==="sniper"||e.type==="bomber"||e.elite))continue;
      const sx=e.x-camX,sy=e.y-camY;if(sx>18&&sx<W-18&&sy>18&&sy<H-18)continue;
      const dx=sx-W/2,dy=sy-H/2;const len=Math.hypot(dx,dy)||1;const edge=14;const ex=Math.max(edge,Math.min(W-edge,W/2+dx/len*Math.min(W/2-edge,Math.abs(W/2/(dx/len)))));const ey=Math.max(edge,Math.min(H-edge,H/2+dy/len*Math.min(H/2-edge,Math.abs(H/2/(dy/len)))));
      ctx.save();ctx.translate(ex,ey);ctx.rotate(Math.atan2(dy,dx));ctx.globalAlpha=0.65+0.2*Math.sin(this.aT*8);ctx.fillStyle=e.elite?"#ef4444":e.type==="sniper"?"#dc2626":"#f97316";ctx.beginPath();ctx.moveTo(8,0);ctx.lineTo(-5,-5);ctx.lineTo(-5,5);ctx.closePath();ctx.fill();ctx.restore();ctx.globalAlpha=1;
    }
    // Arena hazards
    ...
    if(this.eventEnd>this.time&&this.eventKind==="bloodmoon"){
      ctx.globalAlpha=0.08+0.03*Math.sin(this.aT*3);ctx.fillStyle="#ef4444";ctx.fillRect(0,0,W,H);ctx.globalAlpha=1;
    }
  ...
  private loop(now:number){this.raf=requestAnimationFrame(t=>this.loop(t));let dr=(now-this.lt)/1000;this.lt=now;if(dr>0.05)dr=0.05;if(dr<=0)dr=0.0001;this.aT+=dr;
    ...
    if(this.phase==="playing")this.update(dt,dr);else if(this.phase==="menu"){this.cam.x=Math.cos(this.aT*0.07)*260;this.cam.y=Math.sin(this.aT*0.052)*260;this.upFx(dr);}
      ...
      // Simulation pauses in overlays, but feel timers still need to settle.
      this.trauma=Math.max(0,this.trauma-dr*3.8);
      this.upFx(this.phase==="gameover"?dr*0.4:dr*0.12);
    }
    this.render();this.upHud(dr);}
}
