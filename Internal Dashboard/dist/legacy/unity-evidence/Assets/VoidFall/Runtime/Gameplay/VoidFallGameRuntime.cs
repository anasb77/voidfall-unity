...
namespace VoidFall.Runtime
{
    ...
    private Action<int, int, bool> _pickupCollectedHook;
        ...
        private void OnDestroy()
        {
            ...
            if (_arenaResidency != null) ReconcileArenaResidency(default);
        ...
        private void Update()
        {
            ...
            if (keyboard != null && _ui?.QuitConfirm?.IsVisible != true)
            {
                ...
                // Submit belongs to the focused uGUI control. Reading Enter /
                // Space here as Start would activate Home behind a modal.
        ...
        private void ReadNavigationShortcuts()
        {
            var keyboard = Keyboard.current;
            var pad = Gamepad.current;
            var cancel = keyboard?.escapeKey.wasPressedThisFrame == true || pad?.buttonEast.wasPressedThisFrame == true;
            var pause = keyboard?.pKey.wasPressedThisFrame == true || pad?.startButton.wasPressedThisFrame == true;
            var map = keyboard?.tabKey.wasPressedThisFrame == true || pad?.selectButton.wasPressedThisFrame == true;
            if (_ui?.QuitConfirm?.IsVisible == true)
            {
                if (cancel) _ui.QuitConfirm.Hide();
                return;
            }
            if (_dealerOpen)
            {
                if (cancel || pause) CloseDealer();
                return;
            }
            if (_routeMapOpen)
            {
                if (cancel || map) CloseRouteMap();
                return;
            }
            if (_revivePending || _prizeRevealActive || _rouletteActive || _levelUpActive) return;
            if (_menuPage != MenuPage.None)
            {
                if (cancel) CloseMenu();
                return;
            }
            if (map) ToggleRouteMap();
            else if (cancel || pause) TogglePause();
        }
        ...
        private void LogSlowStartupPhase(string phase, double started)
        {
            ...
            if (_startupMenuReportLogged || _startupMenuReadyRealtime <= 0 ||
                Time.realtimeSinceStartupAsDouble - _startupMenuReadyRealtime >= 10.0) return;
        ...
        {
            if (_saveStore?.UnsupportedSaveVersion > SaveStore.SaveVersion)
            {
                SetMenuNotice("This profile needs a newer game version. Your save is preserved.");
                return;
            }
        ...
        {
            _gameSim.ClearHostileShots();
            for (var index = 0; index < _hostileShotViews.Length; index++) Hide(_hostileShotViews[index]);
        }
        ...
        private UnityTelemetryProgress BuildTelemetryProgress()
        {
            ...
            return new UnityTelemetryProgress
            {
                ...
                dealerShield = _dealerShield, shieldCapacity = _shieldCapacity,
                lifeStealKillProgress = _lifeStealKillProgress, scavengerScrapProgress = _scavengerScrapProgress,
                ordinaryScrapDropChance = SurvivalSupportRules.ScrapChance(SupportRank("scavenger")),
                delayedPowerCombatSeconds = _dealerDelayedOwned ? _dealerCombatSeconds : -1,
        ...
        private static int BuildChipIconSlot(string id)
        {
            ...
            if (id == "secondWind" || id == "lifeSteal") return 13;
            if (id == "scavenger") return 12;
        ...
        private void SetupCamera()
        {
            ...
            // The scene's legacy camera disabled the URP asset's authored 2x
            // multisampling. Apply the same policy to existing and new cameras.
        ...
        private sealed class RuntimeGameBridge : IGameBridge
        {
            ...
            public bool TryCommitProfile(SaveData candidate) => _rt.TryCommitProfile(candidate);
